...
const state={origin:'kamata',time:'90',active:null,draws:0,sessionRejected:new Set(),heroTimer:null,drawBusy:false,heroRequestToken:0,scrollTimer:null,heroLoadTimer:null};
...
function visualFor(r){
  ...
  if(record){
    ...
    const selected=heroAssetSelection(record);
...
function startHeroClock(r){
  if(state.heroTimer)clearInterval(state.heroTimer);
  updateHeroTimes(r);
  state.heroTimer=setInterval(()=>state.active&&updateHeroTimes(state.active),30000);
}

function atlasPointsFor(r){
  const nodes=[...(r.waypoints||[]),r.destination].filter(Boolean);
  const count=Math.max(2,nodes.length+1);
  const seed=hash(String(r.id||r.destination));
  return Array.from({length:count},(_,i)=>{
    const t=i/(count-1);
    const x=70+t*665;
    const wave=Math.sin(t*Math.PI*2+(seed%17)/5)*55;
    const bend=Math.sin(t*Math.PI)*(((seed>>3)%95)-47);
    const y=320-t*225+wave+bend;
    return{x:Math.round(x),y:Math.max(62,Math.min(350,Math.round(y)))};
  });
}
function atlasPath(points){
  if(points.length<2)return'';
  let d=`M${points[0].x} ${points[0].y}`;
  for(let i=1;i<points.length;i++){
    const p0=points[i-1],p=points[i],mx=Math.round((p0.x+p.x)/2);
    d+=` C${mx} ${p0.y}, ${mx} ${p.y}, ${p.x} ${p.y}`;
  }
  return d;
}
function renderAtlasRoute(r){
  const points=atlasPointsFor(r),d=atlasPath(points);
  $('#atlas-route-shadow').setAttribute('d',d);
  $('#atlas-route-line').setAttribute('d',d);
  $('#atlas-route-dots').innerHTML=points.map((p,i)=>`<circle cx="${p.x}" cy="${p.y}" r="${i===0?9:i===points.length-1?13:6}" class="${i===0?'start-dot':i===points.length-1?'end-dot':'stop-dot'}"/>`).join('');
}
function categoryFallbackVisual(r){
  const d=destMeta(r),cat=category(r,d),a=VISUALS[cat]||VISUALS.city;
  return 'assets/visuals/'+a[hash(normalize(r.destination))%a.length];
}
function applyHeroImage(r){
  const image=$('#hero-image');
  const primary=visualFor(r);
  const fallback=categoryFallbackVisual(r);
  const token=++state.heroRequestToken;
  if(state.heroLoadTimer)clearTimeout(state.heroLoadTimer);
  image.dataset.primarySrc=primary;
  image.dataset.heroLoading='true';
  const finishTimer=()=>{if(state.heroLoadTimer){clearTimeout(state.heroLoadTimer);state.heroLoadTimer=null}};
  const commit=(src,mode)=>{
    if(token!==state.heroRequestToken)return;
    finishTimer();
    image.src=src;
    image.alt=heroAltFor(r.destination);
    image.dataset.heroMode=mode;
    image.dataset.heroLoading='false';
  };
  const fail=()=>{
    if(token!==state.heroRequestToken)return;
    finishTimer();
    image.dataset.heroLoading='failed';
    image.dataset.heroMode='previous_preserved';
  };
  const loadFallback=(mode)=>{
    if(token!==state.heroRequestToken)return;
    const retry=new Image();
    retry.decoding='async';
    retry.onload=()=>commit(fallback,mode);
    retry.onerror=fail;
    state.heroLoadTimer=setTimeout(fail,3500);
    retry.src=fallback;
  };
  const preload=new Image();
  preload.decoding='async';
  preload.onload=()=>commit(primary,heroRecordFor(r.destination)?'destination':'category_fallback');
  preload.onerror=()=>loadFallback('asset_error_fallback');
  state.heroLoadTimer=setTimeout(()=>loadFallback('asset_timeout_fallback'),3500);
  preload.src=primary;
}
...
function render(r){state.active=r;state.draws++;const d=destMeta(r);startHeroClock(r);applyHeroImage(r);$('#hero-copy').textContent=story(r,d);$('#destination-name').textContent=r.destination;$('#hero-tags').innerHTML=tags(r,d).map(x=>`<span>${esc(x)}</span>`).join('');const url=mapsUrl(r);$('#maps-primary').href=url;$('#maps-atlas').href=url;$('#mobile-maps-primary').href=url;$('#route-role').textContent=sourceRole(r).toUpperCase();$('#detail-tags').innerHTML=tags(r,d).slice(0,3).map(x=>`<span>${esc(x)}</span>`).join('');$('#route-title').textContent=r.title||`${r.destination}へ直行`;$('#route-intent').textContent=r.intent||d.reason||'到着までの景観の変化を一本の体験として楽しむ。';const wp=[...(r.waypoints||[]),r.destination];$('#waypoint-list').innerHTML=wp.map((x,i)=>`<li><small>${i===wp.length-1?'DESTINATION':'STOP '+(i+1)}</small><br>${esc(x)}</li>`).join('');$('#fact-time').textContent=timeLabel().replace('移動','');$('#fact-stops').textContent=`${wp.length}地点`;$('#fact-score').textContent=`${Math.round(score(r))}%`;$('#caution-box').textContent=r.caution||d.caution||'営業時間・駐車条件・通行規制を出発前に確認してください。';$('#atlas-origin').textContent=state.origin==='kamata'?'蒲田駅':'現在地';$('#atlas-destination').textContent=r.destination;$('#atlas-stops').innerHTML=wp.map(x=>`<span>${esc(x)}</span>`).join('');renderAtlasRoute(r);$('#mission').classList.remove('hidden');$('#details').classList.remove('hidden');$('#mobile-action-dock').classList.remove('hidden');$('#desktop-reroll').classList.remove('hidden');remember(r);emitNativeEvent('pc:proposal',buildProposalPayload(r));if(state.scrollTimer)clearTimeout(state.scrollTimer);state.scrollTimer=setTimeout(()=>$('#mission').scrollIntoView({behavior:'smooth',block:'start'}),80)}
function setDrawBusy(busy){
  state.drawBusy=busy;
  ['launch-button','hero-reroll','sticky-reroll','desktop-reroll'].forEach(id=>{const el=$('#'+id);if(el)el.disabled=busy});
  $('#mission').setAttribute('aria-busy',String(busy));
}
function draw({rejectCurrent=false}={}){
  if(state.drawBusy)return false;
  setDrawBusy(true);
  try{
    if(rejectCurrent)emitNativeEvent('pc:reroll',{previousRouteId:state.active?.id||null});
    if(rejectCurrent&&state.active){state.sessionRejected.add(state.active.id);bump(state.active.id,'rejected')}
    let pool=eligible();
    if(!pool.length){state.sessionRejected.clear();pool=eligible()}
    const r=weightedPick(pool);
    if(r)render(r);
    return Boolean(r);
  }finally{
    setTimeout(()=>setDrawBusy(false),420);
  }
}
...
function updatePool(){const p=eligible(),unique=new Set(p.map(r=>r.destination)).size,verified=p.filter(r=>r.candidateSource==='canonical_route').length;$('#pool-status').textContent=`選択した所要時間で${unique}種類／${p.length}通り（検証済みルート${verified}件）。`}
...
window.PC_RUNTIME=Object.freeze({
  draw:()=>draw(),
  reroll:()=>draw({rejectCurrent:true}),
  snapshot:()=>({routeId:state.active?.id||null,destination:state.active?.destination||null,draws:state.draws,time:state.time,origin:state.origin})
});
